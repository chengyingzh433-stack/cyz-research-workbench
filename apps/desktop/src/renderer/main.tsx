...
function App(){
  ...
  async function choose(){if(dirty&&!confirm('草稿有未保存修改，是否放弃这些修改并切换项目？'))return;const data=await call(api.projects.choose());if(data){setSnapshot(data);hydrate(data);setDirty(false);setDecision(null);setUsage(null)}}
  ...
  async function selectView(next:string){if(next===view)return;if(dirty&&!confirm('是否放弃尚未保存的草稿修改？'))return;if(next==='draft'&&projectId)await loadDraft();setDirty(false);setView(next);if(next==='history'&&projectId)setHistory(await call(api.artifacts.history(projectId,draftPath)));if(next==='results')await loadCandidates(snapshot?.tasks.filter(t=>t.status==='completed').at(-1)?.id??'')}
  ...
  {view==='materials'&&<><div className="toolbar"><strong>项目材料</strong><button onClick={()=>void safe(async()=>{const results=await call(api.sources.import(projectId));setNotice(`已处理 ${results.length} 份材料`);await refresh()})}>＋ 导入材料</button></div>{snapshot.sources.length?snapshot.sources.map(source=><article className="material" key={source.id}><div className="file-icon">{source.name.split('.').at(-1)?.toUpperCase()}</div><div><h3>{source.name}</h3><p>{(source.bytes/1024).toFixed(1)} KB · 原件副本已保存</p><span className="badge">等待解析与阅读</span></div><button onClick={()=>void safe(async()=>{await call(api.sources.open(projectId,source.id))})}>查看原件</button></article>):<div className="empty"><h2>先放进一份研究材料</h2><p>支持 PDF、Markdown、文本和图片。重复内容会复用，同名的不同文件会分开保存。</p></div>}</>}
  ...
  {view==='matrix'&&<div className="empty"><h2>证据矩阵尚待整理</h2><p>文献核验后，再比较研究问题、方法、发现和局限。未知的原文位置不会自动补写。</p></div>}
