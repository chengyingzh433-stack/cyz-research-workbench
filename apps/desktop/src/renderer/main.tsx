...
function App(){
  ...
  return <><header className="app-header"><img className="brand-mark" src="./icons/book.svg" alt="CYZ 书本图标" style={{background:'transparent',borderRadius:0,flexShrink:0}}/><div><strong>CYZ 研究工作台</strong><small>让研究过程与证据留在一起</small></div><div className="header-project">{snapshot?.project.name??'尚未打开项目'}</div><button onClick={()=>void safe(choose)}>打开或新建项目</button><button onClick={()=>setChatOpen(!chatOpen)}>{chatOpen?'收起对话':'展开对话'}</button></header>
