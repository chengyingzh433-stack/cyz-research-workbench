...
contextBridge.exposeInMainWorld('cyz',{
  ...
  parsing:{list:(id:string)=>call('parsing.list',id),read:(id:string,target:string)=>call('parsing.read',id,target),start:(id:string,source:string)=>call('parsing.start',id,source),poll:(id:string,target:string)=>call('parsing.poll',id,target),approve:(id:string,target:string)=>call('parsing.approve',id,target),reconcile:(id:string,target:string)=>call('parsing.reconcile',id,target)},
